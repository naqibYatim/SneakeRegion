...
<body bgcolor="#acb2ff">
	...
	<div id="demoFont">Converse Men's Shoes</div>
		...
		<?php
			...
			$query = "SELECT pd_id, pd_image, max(pd_brand), max(pd_price), max(pd_name), max(pd_quantity), max(pd_category), max(pd_size)
						FROM product
						WHERE product.pd_brand='CONVERSE'
						AND product.pd_category='men'
						GROUP BY pd_image;";
			$result = mysqli_query($dbc, $query);
			
			echo "<form action='converse_men.php' method='post' enctype='multipart/form-data'>";
