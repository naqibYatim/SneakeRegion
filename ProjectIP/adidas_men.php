...
<body bgcolor="#acb2ff">
	...
	<div id="demoFont">Adidas Men's Shoes</div>
	
		<?php
			
			require_once 'connect.php';
			/*
			SELECT pd_image, max(pd_brand), max(pd_price), max(pd_name), max(pd_quantity), max(pd_category), max(pd_size)
			FROM product
			WHERE product.pd_brand='ADIDAS'
			AND product.pd_category='men'
			GROUP BY pd_image;
			*/
			...
			echo "<form action='adidas_men.php' method='post' enctype='multipart/form-data'>";
